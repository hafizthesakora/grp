...
// TODO: Replace with backend/CMS data when ready
const ESTATE_BANNER = {
  badge: "Now Selling",
...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      {/* Estate announcement banner */}
      {showBanner && (
        <div className="bg-green-950 border-b border-gold-400/20">
          <div className="max-w-[1440px] mx-auto px-8 lg:px-20 py-2 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <span className="text-white text-sm font-semibold truncate">{ESTATE_BANNER.estate}</span>
      ...
      <header
        className={`transition-all duration-500 ${
          isTransparent
            ? "bg-transparent"
            : "bg-green-950 shadow-xl shadow-black/20"
        }`}
      >
      <div className="max-w-[1440px] mx-auto px-8 lg:px-20 flex items-center justify-between h-20 lg:h-[112px]">

        {/* Logo — text when transparent over hero video, image when scrolled */}
        <Link href="/" className="flex items-center shrink-0">
          {isTransparent ? (
            <div className="flex flex-col leading-none select-none">
              <span className="text-gold-400 font-extrabold tracking-tight" style={{ fontSize: "clamp(20px, 1.8vw, 26px)" }}>
                GOLDEN ROOTS
              </span>
              <span className="text-white/65 font-bold uppercase tracking-[0.2em]" style={{ fontSize: "clamp(10px, 0.75vw, 12px)" }}>
                Properties
              </span>
            </div>
          ) : (
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden bg-green-950 border-t border-white/10 px-8 py-5 space-y-1">
          {navLinks.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              onClick={() => setOpen(false)}
              className={`block py-3 text-sm font-semibold border-b border-white/5 transition-colors ${
          ...
          <div className="pt-4">
            <Link href="/purchase" onClick={() => setOpen(false)}
                  className="block text-center py-3 bg-gold-400 text-green-950 font-bold text-sm">
              Get Started
            </Link>
          </div>
        </div>
